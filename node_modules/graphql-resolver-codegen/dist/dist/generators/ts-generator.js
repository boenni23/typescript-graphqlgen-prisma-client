...
function generate(args) {
    ...
    var unions = args.unions;
    return "\nimport { GraphQLResolveInfo } from 'graphql'\n\nexport interface ResolverFn<Root, Args, Ctx, Payload> {\n  (root: Root, args: Args, ctx: Ctx, info: GraphQLResolveInfo):\n    | Payload\n    | Promise<Payload>\n}\n\nexport interface ITypes {\nContext: any\n" + enums.map(function (e) { return e.name + "Root: any"; }).join(os.EOL) + "\n" + unions.map(function (union) { return union.name + "Root: any"; }).join(os.EOL) + "\n" + types.map(function (type) { return type.name + "Root: any"; }).join(os.EOL) + "\n}\n\n  " + types
        .map(function (type) {
        return "export namespace I" + type.name + " {\n  " + type.fields
            .map(function (field) {
            return (field.arguments.length > 0
                ? "export interface Args" + capitalize(field.name) + " {\n      " + field.arguments
                    ...
                    return arg.name + ": " + (source_helper_1.GraphQLScalarTypeArray.indexOf(arg.type.name) > -1
                        ? source_helper_1.getTSTypeFromGraphQLType(arg.type.name)
                ...
                ? source_helper_1.getTSTypeFromGraphQLType(field.type.name)
                : "T['" + field.type.name + "Root']") + (field.type.isArray ? "[]" : "") + "\n  >\n  ";
        })
